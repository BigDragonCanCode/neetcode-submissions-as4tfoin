...
public:
    string minWindow(string s, string t) {
        ...
        while (right < s.size()) {
            ...
            while (have == t.size()) {
                freq[s[left]]--;
                //the second if is for the case:
                //need: ABC current string: AAABC
                //move left is still valid, it only becomes invalid when the window
                //occurrence is less than the needed occurrence
